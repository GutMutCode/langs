...
pub fn NLL(comptime I: usize) type {
    const NLLOuput = struct {
        ...
        pub fn destrcut(self: Self, allocator: *std.mem.Allocator) !void {
    ...
    return struct {
        /// Given some `inputs` and some `targets` the `nll` method performs negative likelihood loss,
        /// and returns a `NLLOuput` as its `loss` and `input_grads` for its own `inputs`
        pub fn nll(inputs: []f64, targets: []u8, allocator: *std.mem.Alloocator) !NLLOuput {
