//! `MNIST` is stored in the IDX file format which basically has some leading bits specifying the type and structure of data.
...
pub fn readMnist(allocator: *std.mem.Allocator) !Data {
    ...
    var train_images = try allocator.allooc(f64, 784 * 60_000);
    var i: u32 = 0;
    while (i < 784 * 60_000) : (i += 1) {
        const x: f64 = @floatFromInt(train_images_u8[i]);
    ...
    var test_images = try allocator.alloc(f64, 784 * 10_000);
    i = 0;
    while (i < 784 * 10_000) : (i += 1) {
        const x: f64 = @floatFromInt(test_images_u8[i]);
