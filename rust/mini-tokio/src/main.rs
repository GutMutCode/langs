...
// A utility that allows us to impement a `std::task::Waker` without having to
// use `unsafe` code.
...
fn main() {
    ...
    ///
    /// When a task is executd, the send half of the channel is passed along via
    /// the task's Waker.
    struct MiniTokio {
    ...
    impl MiniTokio {
        ...
        ///
        /// Tasks are popped from the `scheduled` channel redeiver. Receiving a task
        ...
        fn run(&self) {
            ...
            // entiering the runtime, the executor stores necessary context with the
    ...
    async fn delay(dur: Duration) {
        ...
        struct Delay {
            ...
            // accessable by both the timer thread and the future so it is wrapped
            // with `Arc<Mutex<_>>`.
        ...
        impl Future for Delay {
            ...
            fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
                ...
                if let Some(waker) = &self.waker {
                    ...
                    // This is necessary as the `Delay` furture instance may move to
                ...
                // the future has competed and `Poll::Ready` is returned.
